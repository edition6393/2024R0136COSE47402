...
class ResidualAttentionBlock_MYTEMP(nn.Module):
    ...
    def forward(self, inputs, attn_mask: torch.Tensor):
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        ##ignore surfix for this paper and add learnable tokens to the end of prompt
                        x = torch.cat([prefix, textual_context], dim=0)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer_MYTEMP(
                input_resolution=image_resolution,
